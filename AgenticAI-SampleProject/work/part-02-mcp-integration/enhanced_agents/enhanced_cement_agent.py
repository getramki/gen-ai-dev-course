...
class EnhancedCementAgent(CementAgent):
    ...
    async def _call_mcp_tool(self, tool_name: str, **kwargs) -> Dict:
        """Call MCP tool via proper MCP client"""
        try:
            from mcp_client import call_cement_tool
            return await call_cement_tool(tool_name, **kwargs)
        except Exception as e:
            print(f"⚠️ Cement MCP tool call failed: {e}")
            return {"error": str(e)}
    
    async def _calculate_optimal_margin_mcp(self, state: EnhancedCementState) -> EnhancedCementState:
    ...
    async def _analyze_pricing_strategy_mcp(self, state: EnhancedCementState) -> EnhancedCementState:
        ...
        try:
            ...
            if "error" not in result:
                ...
                if result.get("recommended_price", 0) > 0:
                    if "current_offer" not in state or not state["current_offer"]:
                        state["current_offer"] = {}
    ...
    async def _check_inventory_mcp(self, state: EnhancedCementState) -> EnhancedCementState:
        ...
        try:
            ...
            if "error" not in result:
                ...
                if "current_offer" in state and state["current_offer"]:
                    if result.get("inventory_status") == "critical":
                        # Premium pricing for low inventory
                        current_price = state["current_offer"].get("price_per_ton", 180)
    ...
    def _analyze_competition_enhanced(self, state: EnhancedCementState) -> EnhancedCementState:
        ...
        if state["competitor_offers"]:
            ...
            our_price = state["current_offer"].get("price_per_ton", 180)
            competitor_prices = [offer.get("price_per_ton", float('inf')) for offer in state["competitor_offers"]]
    ...
    async def _calculate_competitive_response_mcp(self, state: EnhancedCementState) -> EnhancedCementState:
        """Calculate competitive response using MCP tools"""
        if not state["competitor_offers"]:
            return state
        
        our_current_price = state["current_offer"].get("price_per_ton", 180)
    ...
    async def _finalize_offer_enhanced(self, state: EnhancedCementState) -> EnhancedCementState:
